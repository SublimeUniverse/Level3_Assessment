...
class Play:
    ...
    def generate_rounds(self):
        ...
        first_number_addsub = random.randint(0, 30)
        second_number_addsub = random.randint(0, 30)

        # Numbers for multiplication and division
        first_number_muldiv = random.randint(1,12)
        second_number_muldiv = random.randint(1,12)

        if level == 1:
            operation = random.choice(operations)

            if operation == "+":
                self.result = first_number_addsub + second_number_addsub
            else:
                if operation == "-":
                    self.result = first_number_addsub - second_number_addsub
            ...
            operation = "x"
            self.result = first_number_muldiv * second_number_muldiv

        else:
            operation = "÷"
            self.result = first_number_muldiv * second_number_muldiv
            first_number_muldiv = self.result
            self.result = first_number_muldiv




        # Get first or second number depending on level for displaying question
        first_number = first_number_addsub
        first_number = first_number_muldiv
        second_number = second_number_addsub
        second_number = second_number_muldiv
