...
def check_rounds(number, level, infinite, class_type, button_config, label):
    """
    Check users have entered 1 or more
    """

    # get rounds wanted
    rounds_input = number.get().strip()

    if rounds_input == "" and infinite == "yes":
        ...
        try:
            ...
            button_config.config(fg="red")
            return

    number.delete(0, END)
    button_config.config(text=label, fg="#000000")
    if level != "":
        class_type(rounds_wanted, level)
    else:
        Play.check_answer()
    root.withdraw()
...
class Start:
    def __init__(self):
        ...
        self.first_button = Button(self.start_frame, width=30, height=2,
                                   ...
                                   command=lambda: to_check_rounds(1))
        ...
        self.second_button = Button(self.start_frame, width=30, height=2,
                                    ...
                                    font=("Arial", 9, "bold"), command=lambda: to_check_rounds(2))
        ...
        self.third_button = Button(self.start_frame, width=30, height=2,
                                   ...
                                   font=("Arial", 9, "bold"), command=lambda: to_check_rounds(3))
        ...
        def to_check_rounds(level):
            check_rounds(self.num_rounds_entry, level,
                         "yes", Play, self.choose_label, self.label, )
...
class Play:
    def __init__(self, how_many, level):
        ...
        # Display right or wrong, above go button so it can be configured as a error box
        ...
        self.go_button = Button(self.entry_area_frame, font=("Arial", 14, "bold"),
                                width=5 , text="GO", bg="#60A917", fg="#FFFFFF",
                                command=check_rounds(f"{self.enter_answer}",1, "no",
                             Play, self.display_area, ""))
