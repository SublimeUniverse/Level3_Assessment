...
class Instructions:
    def __init__(self, partner):
        ...
        # All instructions labels
        instructions_labels_list = [

            # Text | font
            [" Instructions ", ("Arial", 20, "bold")],
            [f"{instructions_text.instructions}", ("Arial", 10, "bold")],
        ...
        for count, item in enumerate(instructions_labels_list):
            make_label = Label(self.start_frame, text=item[0], font=item[1],  bg="#FAD7AC",
...
class Play:
    def __init__(self, how_many, level):
        ...
        # List for labels details (Frame | text | font | background |foreground | row | command | width | pady | state)
        play_buttons_list = [
            [self.entry_area_frame, "GO", ("Arial", 14, "bold"), "#60A917", "#FFFFFF",2, self.check_answer, 5, 5,"normal", 1],
            [self.start_frame,"Next Round", ("Arial", 15, "bold"), "#F5F5F5",  "#FFFFFF", 4, self.next_round, 20,10 ,"disabled", None],
            [self.start_frame,"Statistics", ("Arial", 15, "bold"), '#1BA1E2', "#FFFFFF", 5, lambda: self.to_stats(), 20,10, "disabled", None],
            [self.start_frame,"End Game", ("Arial", 15, "bold"), "#E51400", "#FFFFFF", 6, self.close_play, 20, None, "normal", None]

        ]

        play_buttons_ref = []
        for item in play_buttons_list:
            self.make_button = Button(item[0], text=item[1], font=item[2]
                                    , bg=item[3],fg=item[4], wraplength=300, justify="left", width=item[7],
                                      command=item[6], state=item[9])
            self.make_button.grid(row=item[5], pady=item[8], column=item[10])

            play_buttons_ref.append(self.make_button)

        self.stats_button = play_buttons_ref [2]
        self.next_round_button = play_buttons_ref[1]
        self.go_button = play_buttons_ref[0]
